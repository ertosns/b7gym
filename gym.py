...
def check_date():
    ...
    for registration in registrations:
        ...
        end_date=registration[12]
        contact_no=registration[7]
...
class MainApp(ctk.CTk):
    def __init__(self):
        ...
        self.logo_image=ctk.CTkImage(
            light_image=Image.open(os.path.join(image_path, "resized-logo.png")),
            dark_image=Image.open(os.path.join(image_path, "resized-logo.png")),
            size=(150, 60))
...
def create_home_frame(home):
    ...
    def get_members_count():
        ...
        conn=sqlite3.connect('SQLite db/registration_form.db')
        cursor=conn.cursor()
        print("account manager: {}".format(current_account_manager))
        if current_account_manager == ADMIN_MANAGER:
            cursor.execute("SELECT COUNT(*) FROM registration")
        else:
            cursor.execute("SELECT COUNT(*) FROM registration WHERE full_name=?", (current_account_manager,))
...
class RegistrationFrame(ctk.CTkFrame):
    def __init__(self, master, data=None, **kwargs):
        ...
        label=ctk.CTkLabel(self, text="B7 Gym Membership Registration", font=("Arial bold", 26))
        ...
        contact_no_entry=ctk.CTkEntry(contact_frame, placeholder_text="+20101234567")
        ...
        # Create the widgets for subscription period, start date, and end date
        ...
        try:
            ...
            print("Error: ", e)
            messagebox.showerror("Account management")

        account_manager_label=ctk.CTkLabel(subscription_frame, text="Account Manager:", font=label_font)
        account_manager_label.grid(row=6, column=0, padx=20, pady=10, sticky="w")
        ...
        take_button=ctk.CTkButton(subscription_frame, text="Take Photo", command=self.take_photo)
        upload_button=ctk.CTkButton(subscription_frame, text="Upload Photo", command=self.upload_photo)
        upload_button.grid(row=3, column=0, padx=20, pady=10)

        take_button.grid(row=3, column=1, padx=20, pady=10, sticky="w")


        # Uploaded photo entry
        self.uploaded_photo_entry=ctk.CTkEntry(subscription_frame, placeholder_text=".png/.jpg/etc")
        self.uploaded_photo_entry.grid(row=3, column=2, padx=20, pady=10)
        ...
        cursor.execute('''
                   CREATE TABLE IF NOT EXISTS registration (
                       ...
                       week_mode REAL,
                       cardio_mode REAL,
                       FOREIGN KEY(full_name) REFERENCES accounts(full_name)
                   )
               ''')
    ...
    # Function to take photo using computer cam
    def take_photo(self):
        ...
        while True:
            ...
            if  keypress == ord('s'):
                cv2.imwrite("./templates/member_profile/image.jpeg", frame)
                try:
                    # TODO set insert image from memory
                    self.uploaded_photo_entry.delete(0, tk.END)
                    self.uploaded_photo_entry.insert(0, os.path.basename("image.jpeg"))
                    break
                except Exception as e:
    ...
    def register_subscription(self):
        ...
        week_mode = self.week_mode_entry.get()
        cardio_mode = self.cardio_mode_entry.get()
        full_name = self.account_manager_entry.get()

        # Validate the data
        if not (first_name and last_name and age and sex and birth_date and
                contact_no and email and subscription_period and
                week_mode and cardio_mode):
        ...
        if len(contact_no) != 12:
            messagebox.showerror("Validation Error", "Contact No must be an 12-digit number.")
            return
        ...
        cursor.execute('''
           INSERT INTO registration (first_name, middle_name, last_name, age, sex, birth_date, contact_no, email, subscription_id, subscription_period, start_date, end_date, photo_data, week_mode, cardio_mode, full_name)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (first_name, middle_name, last_name, age, sex, birth_date,contact_no,
              email, subscription_id, subscription_period, start_date_str, end_date_str,
              sqlite3.Binary(photo_data), week_mode, cardio_mode, full_name))
...
class ViewFrame(ctk.CTkFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        if current_account_manager==ADMIN_MANAGER:
            cursor.execute("SELECT id, first_name, middle_name, last_name, contact_no, subscription_id, start_date, end_date, status, week_mode, cardio_mode, full_name FROM registration")
        else:
            print(current_account_manager)
            cursor.execute("SELECT id, first_name, middle_name, last_name, contact_no, subscription_id, start_date, end_date, status, week_mode, cardio_mode, full_name  FROM registration WHERE full_name=?", (current_account_manager,))
        ...
        self.table=ttk.Treeview(table_frame, columns=(
            ...
            "Start Date", "End Date", "Status", "Machines", "Cardio", "Manager"), show="headings", height=11)
        ...
        self.table.heading("Machines", text="Machines")
        self.table.heading("Cardio", text="Cardio")
        self.table.heading("Manager", text="Manager")
    ...
    def search_record(self, search_term):
        ...
        if current_account_manager==ADMIN_MANAGER:
            cursor.execute(
                "SELECT id, first_name, middle_name, last_name, contact_no, subscription_id, start_date, end_date, status, week_mode, cardio_mode FROM registration WHERE first_name LIKE ? OR last_name LIKE ? OR status LIKE ? OR contact_no LIKE ? OR subscription_id LIKE ? OR sex LIKE ?",
                ('%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', search_term ))
        else:
            cursor.execute(
                "SELECT id, first_name, middle_name, last_name, contact_no, subscription_id, start_date, end_date, status, week_mode, cardio_mode, full_name FROM registration WHERE full_name=? AND (first_name LIKE ? OR last_name LIKE ? OR status LIKE ? OR contact_no LIKE ? OR subscription_id LIKE ? OR sex LIKE ?)",
                (current_account_manager, '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', search_term))
        records=cursor.fetchall()
    ...
    def refresh_table(self):
        ...
        if current_account_manager==ADMIN_MANAGER:
            cursor.execute(
                "SELECT id, first_name, middle_name, last_name, contact_no, subscription_id, start_date, end_date, status, week_mode, cardio_mode FROM registration")
        else:
            cursor.execute(
                "SELECT id, first_name, middle_name, last_name, contact_no, subscription_id, start_date, end_date, status, week_mode, cardio_mode FROM registration WHERE full_name=?", (current_account_manager,))
        records=cursor.fetchall()
...
class EditForm(ctk.CTkToplevel):
    def __init__(self, master, first_name, id_value, table_reference):
        ...
        if current_account_manager==ADMIN_MANAGER:
            ...
            self.cursor.execute("SELECT * FROM registration WHERE full_name=? AND id=?", (current_account_manager, id_value,))
        ...
        photo_blob=self.member_data[-4]  # Assuming the photo is stored in the last column
    ...
    def update_record(self):
        ...
        try:
            ...
            self.cursor.execute('''
                ...
                first_name=?, middle_name=?, last_name=?, age=?, sex=?, birth_date=?,
                contact_no=?, email=?, subscription_id=?, subscription_period=?, start_date=?,
                end_date=?, status=?, week_mode=?, cardio_mode=?
                WHERE id=?
                ...
                if record_data:
                    ...
                    try:
                        if current_account_manager==ADMIN_MANAGER:
                            ...
                            cursor.execute("DELETE FROM registration WHERE full_name=? and id=?", (current_account_manager, id_value,))
...
class RenewSubscriptionFrame(ctk.CTkToplevel):
    def __init__(self, master, id_value, table_reference):
        ...
        # Create labels and entry fields for editing the record
        labels=["ID:", "Contact No:", "Subscription ID:", "Subscription Period:", "Start Date:", "End Date:"]
        self.entry_fields=[]
        ...
        # Fetch data for the specified member using the provided 'id_value'
        self.cursor.execute(
            "SELECT id, contact_no, subscription_id, subscription_period, start_date, end_date FROM registration WHERE id=?",
            (id_value,))
        self.member_data=self.cursor.fetchone()
    ...
    def renew_membership(self):
        ...
        try:
            ...
            #TODO get the renewal duration. make as drop down box with same membership period options to get the period.
        ...
        try:
            ...
            with sqlite3.connect('SQLite db/registration_form.db') as conn_registration:
                ...
                if current_account_manager==ADMIN_MANAGER:
                    ...
                    SELECT status, end_date
                    FROM registration
                    WHERE full_name=? AND subscription_id = ?
                    ''', (full_name, subscription_id,)).fetchone()
...
class TrainerFrame(ctk.CTkFrame):
    ...
    def register_subscription(self):
        ...
        cursor.execute('''
            INSERT INTO trainer (
                first_name, middle_name, last_name, age, sex, birth_date,
                contact_no, status, photo_data
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            first_name, middle_name, last_name, age, sex, birth_date,
            contact_no, status, sqlite3.Binary(photo_data)))
...
class ViewTrainerFrame(ctk.CTkFrame):
    ...
    def search_record(self, search_term):
        ...
        # Search for records that match the search term
        cursor.execute(
            "SELECT id, first_name, middle_name, last_name, age, sex, contact_no, status from trainer WHERE first_name LIKE ? OR last_name LIKE ? OR status LIKE ? OR contact_no LIKE ? OR subscription_id LIKE ? OR sex LIKE ?",
            ('%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%'))
        records=cursor.fetchall()
...
class EditTrainerForm(ctk.CTkToplevel):
    ...
    def update_record(self):
        ...
        updated_data=[entry.get() if label_text != "Status:" else self.status_combobox.get() for label_text, entry in
                      zip(["First Name:", "Middle Name:", "Last Name:", "Age:", "Sex:", "Date of Birth:", "Contact No:",  "Status:"],
                          self.entry_fields)]
        ...
        try:
            with sqlite3.connect('SQLite db/register_trainer.db') as conn:
                ...
                cursor.execute('''
                    UPDATE trainer SET
                    first_name=?, middle_name=?, last_name=?, age=?, sex=?, birth_date=?, contact_no=?, status=?
                    WHERE id=?
...
class LogbookFrame(ctk.CTkFrame):
    ...
    def search_record(self, search_term):
        ...
        # Search for records matching the search term
        cursor.execute(
            "SELECT first_name, middle_name, last_name, contact_no, time_start FROM visitors WHERE first_name LIKE ? OR last_name LIKE ? OR status LIKE ? OR contact_no LIKE ? OR subscription_id LIKE ? OR sex LIKE ?",
            ('%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%'))
        records=cursor.fetchall()
...
class ViewEmployeeFrame(ctk.CTkFrame):
    ...
    def search_record(self, search_term):
        ...
        # Search for records matching the search term
        cursor.execute(
            "SELECT id, first_name, middle_name, last_name, age, sex, contact_no, status FROM employees WHERE first_name LIKE ? OR last_name LIKE ? OR status LIKE ? OR contact_no LIKE ? OR subscription_id LIKE ? OR sex LIKE ?",
            ('%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%', '%' + search_term + '%'))
        records=cursor.fetchall()
...
def register(full_name, contact_no, username, password, contact_no_entry, full_name_entry, username_entry,
    ...
    # Validate the contact number format (12 digits)
    if not (contact_no.startswith('2') and len(contact_no) == 12 and contact_no[1:].isdigit()):
        messagebox.showerror("Invalid Contact Number",
                             "Please enter a valid contact number starting with 2 and 12 digits long (e.g., 201012345634).")
        return
...
def forgot_password():
    ...
    while True:
        ...
        if user_phone_number is None:
            break
        elif user_phone_number.startswith('0') and len(user_phone_number) == 12 and user_phone_number[1:].isdigit():
            conn=sqlite3.connect('SQLite db/registered_users.db')
            cursor=conn.cursor()
...
def create_login_window():
    def login():
        ...
        conn.close()
        if user_data:
            current_account_manager = user_data[1]
    ...
    logo_image=ImageTk.PhotoImage(file="templates/resized-logo.png")
